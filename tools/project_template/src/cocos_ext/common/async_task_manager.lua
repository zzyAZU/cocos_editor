...
-- 注意协程不要调用 opengl 相关的东西， opengl 线程不安全
function coroutine_async_load(doLoadFun, ...)
    local ret = {do_execute(doLoadFun, ...)}
    ...
    if ret[1] then
        ...
        local args = {...}
        add_task(function()
            coroutine.resume(co, doLoadFun(unpack(args, 1, table.maxn(args))))
        end)
        coroutine.yield()
    end
end
