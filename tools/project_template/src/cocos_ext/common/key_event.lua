...
local function _onKeyEvent(genKeys, bIncreI)
    ...
    for _, listPanel in sorted_pairs(_curOrderedPanelList, _cmpKeysLess) do
        ...
        for _, curPanel in ipairs(listPanel) do
            local callbakckInfo = _panelKeyCallbackMap[curPanel]
            local listData = callbakckInfo and callbakckInfo[genKeys]
            if listData then
...
local function _initKeyEvent()
    ...
    g_logicEventHandler:AddCallback('logic_event_restart_app', function()
        if _keyListener then
            g_eventDispatcher:removeEventListener(_keyListener)
            _keyListener = nil
        end
    end)
end
...
function add_common_key_event(keys, bIncre, bSwallowEvent, priority, func)
    ...
    if is_function(bIncre) then
        ...
        bSwallowEvent = true
        priority = 0
    elseif is_function(bSwallowEvent) then
        func = bSwallowEvent
        bSwallowEvent = true
        priority = 0
    elseif is_function(priority) then
        func = priority
        priority = 0
    end
    assert(is_boolean(bIncre))
