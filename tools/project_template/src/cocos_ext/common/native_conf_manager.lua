...
function get_native_conf(name)
    ...
    if ret == nil then
        ...
        local nativeFilePath = nativeConfPath .. name
        ret = table.read_from_encrypted_file(nativeFilePath)
        if ret == nil then
            -- backwards compatibility
            ret = table.read_from_file(nativeFilePath)
            if ret ~= nil then
                bSaveConf = true
            end 
        end
...
function save_native_conf(name)
    ...
    table.write_to_encrypted_file(_nativeConf[name], nativeConfPath .. name)
end
