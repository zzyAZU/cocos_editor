...
-- 修改面板层级
function BasePanel:change_panel_zorder(panel,isbefore)
    if isbefore == nil then
        isbefore = true
    end
    if is_valid_str(panel) then
        panel = g_panel_mgr.get_panel(panel)
    end

    if panel and panel:get_layer():getParent() == self:get_layer():getParent() then
        local index = isbefore and 1 or -1
        local newZOrder = panel:get_layer():getLocalZOrder() + index
        self:get_layer():setLocalZOrder(newZOrder)
    end
end
...
-- override
function BaseDialogPanel:__init__(...)
    BasePanel.__init__(self, ...)

    self._is_esc_close = true
    self:add_key_event_callback('KEY_ESCAPE', function()
        self:esc_key_close()
    end)
end

--  不会处理该面板的esc关闭面板 但会处理该面板下的面板
function BaseDialogPanel:skip_esc_key_event()
    self._is_esc_close = false
    self:set_panel_swallow_key_event(false)
end

-- 不会被处理的面板的esc，下面的面板也不会处理
function BaseDialogPanel:solve_esc_key_event()
    self._is_esc_close = false
    self:set_panel_swallow_key_event(true)
end

-- 按回退键被调用
-- override
function BasePanel:esc_key_close()
    if self._is_esc_close then
        self:close_panel()
    end
end
...
-- override
function BaseDialogPanel:set_hide_close_ani_valid(hideValid)
    self.hideCloseAni = hideValid
end
...
function BaseDialogPanel:on_init_end()
    if self:is_dialog() then
        self:_show_blur_bg()
        self.is_open_ani_compelete = false
        self:_show_open_ani()
    end
end
...
function BaseDialogPanel:register_net_event(net, command, callback, nPriority, bCallbackOnce)
    ...
    local function _callback(...)
        ...
        if self.is_open_ani_compelete ~= false then
            call_func(...)
        else
...
function BaseDialogPanel:on_open_ani_compelete()
    self.is_open_ani_compelete = true
...
function BaseDialogPanel:_show_blur_bg()
    ...
    if self._is_show_blur_bg then
        ...
        local newSprite = cc_utils_get_blur_screen()
