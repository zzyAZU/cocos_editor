        ...
        for name, attr_list in pairs(customizeInfo) do
            ...
            if nodeConf then
                for attr, value in pairs(attr_list) do
                    nodeConf[attr] = value
                    -- 允许动态属性更改节点类型
                    if attr == 'type_name' then
                        nodeConf['__checked__'] = nil
                    end
                end
            end
        end
...
function is_template_valid(conf)
    if is_string(conf) then
        return load_template(conf) ~= constant_uisystem.default_empty_template_conf
    end
...
function is_text_left_to_right_order()
    if _bL2ROrder == nil then
        _bL2ROrder = constant_uisystem.left2right_lang[g_native_conf['cur_multilang_index']] ~= nil
    end
    return _bL2ROrder
end

function is_template_has_ani_info(template)
    if is_string(template) then
        template = load_template(template)
    end

    local function _parse(t)
        local ani_data = t['ani_data']
        if is_table(ani_data) and not table.is_empty(ani_data) then
            return true
        else
            local child_list = t['child_list']
            if child_list then
...
function is_ani_template_valid(conf)
    if is_string(conf) then
        return load_ani_template(conf) ~= constant_uisystem.default_empty_ani_template_conf
    end
