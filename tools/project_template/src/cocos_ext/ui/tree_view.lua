...
function CCTreeView:AddControl(item, parent_item, index, not_refresh)
    ...
    if _touchBg_ and btn then
        ...
        btn.newHandler.OnMouseMove = function(bInside, pos, bFirst)
            if bFirst then
...
function CCTreeView:ExpandItem(item, bExpand, bNotRefresh)
    ...
    item.eventHandler:Trigger('OnExpand', bExpand)
...
function CCTreeView:WrapNodeByLevel(level)
    if level == 0 then
        self:ForEachItem(function(item)
            self:ExpandItem(item, true)
        end)
    else
        local wrapItems = table.copy(self._node._child_item)
        for i = 1, level do
            local childList = {}
            for _, v in ipairs(wrapItems) do
                self:ExpandItem(v, i ~= level)
                table.arr_extend(childList, v._child_item)
            end
            wrapItems = childList
        end
    end
...
function CCTreeView:EnableAutoKeyWrapByLevel(relativePanel)
    for i = 0, 9 do
        local keys = {'KEY_CTRL', 'KEY_K', string.format('KEY_%d', i)}
        relativePanel:add_key_event_callback(keys, function()
            self:WrapNodeByLevel(i)
        end)
    end
end
