...
function table.read_from_encrypted_file(filePath)
    if not g_fileUtils:isFileExist(filePath) then
        return
    end
    ...
    local checkBit = string.sub(str, 1, 4)
    local encrypt_utils = direct_import('cocos_ext.common.encrypt_utils')
    local chunk = encrypt_utils.xor_cipher(string.sub(str, 5))
    if encrypt_utils.hash_str(chunk) ~= checkBit then
        return
    end
...
function table.write_to_encrypted_file(t, filePath, reprFun)
    local writeStr = reprFun and reprFun(t) or repr(t)
    local encrypt_utils = direct_import('cocos_ext.common.encrypt_utils')
    local chunk = encrypt_utils.xor_cipher(writeStr)
    local checkBit = encrypt_utils.hash_str(writeStr)
    g_fileUtils:writeStringToFile(str2binary(checkBit, chunk), filePath)
end
...
function table.arr_insert(t, index, tOther)
    for _, v in ipairs(tOther) do
...
-- 按照某个key大小排序
function table.sort_by_key(t, key, isRase)
    table.sort(t, function(a1, a2)
        if isRase then
...
function table.is_arch_equal(t1, t2)
    for k, v in pairs(t1) do
        if not is_type_arch_equal(t2[k], v) then
            return false
        end
    end
