...
cc.RichLabel.register_custom_format('underline', function(content, color, size, richLabel)
    ...
    if paramText and paramColor and paramOthers then
        text = paramText
        local s_color_hex = "0x" .. paramColor
        if tonumber(s_color_hex) == nil then
            s_color_hex = '0xffffff'
        end
        color = ccc3FromHex(s_color_hex)
    end
...
-- 有以下两种格式，text是要显示的文本，color是文本颜色，otherParamsStringToOnCustomEvent是传给事件的字符串参数
-- #C[clicklabel]eventName::text*color*otherParamsStringToOnCustomEvent#n
-- #C[clicklabel]eventName::text#n
cc.RichLabel.register_custom_format('clicklabel', function(content, color, size, richLabel)
    local node = cc.Node:create()
    ...
    local label = cc.RichLabel:Create(text, size)
    label:setAnchorPoint(ccp(0, 0))
    ...
    if is_valid_str(event) then
        ...
        layer.OnClick = function()
            richLabel.eventHandler:Trigger('OnCustomEvent', 'clicklabel', event, paramOthers)
        end
    end
...
cc.RichLabel.register_custom_format('charmap', function(content, color, size, richLabel)
    ...
    if altasPath and altasPath ~= "" then
        ...
        label:SetPosition(0, (size - newHeight) / 2)
...
-- 图片 #C[charmapbottom]path*str*scale#n,下面对齐
cc.RichLabel.register_custom_format('charmapbottom', function(content, color, size, richLabel)
    local node = cc.Node:Create()
    local altasPath, labelContent, scale = string.match(content, "(.*)%*(.*)%*([%d%.]+)")
    if altasPath and altasPath ~= "" then
        local label = cc.Label:CreateWithCharMap(altasPath)
