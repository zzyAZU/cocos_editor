...
-- if profiler then
--     -- 开启全局的比较耗需要手动开启测试
--     profiler.start('o.txt')
-- end

-- check run loop test
-- if luaext_init_check_run_loop then
--     luaext_init_check_run_loop()
--     rawset(_G, 'luaext_init_check_run_loop', nil)
-- end
...
__G__TRACKBACK__ = function(...)
    ...
    if (g_application:getTargetPlatform() == 0 or bIsShowErrorPanel) and logic_utils_show_error_panel then
...
local function _registerConfig()
    -- for test
    -- 调试模式 在调试模式下脚本配置是可变的
    g_conf_mgr.register_native_conf('debug_mode', true)
