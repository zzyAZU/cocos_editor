...
function EditCtrl:on_update_data()
    ...
    local allMotions = table.keys(motionConfs)
    if not is_valid_str(self._ani_combo_data) or not table.find_v(allMotions, self._ani_combo_data) then
        self._ani_combo_data = allMotions[1]
    	self._data.motion = self._ani_combo_data
        self._editCallback(self._data)
    end 
