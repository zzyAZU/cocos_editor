...
local function _addRecentFile(panel)
    ...
    g_native_conf['anieditor_recent_open_files'] = recentFiles
end
...
function get_recent_open_file_list()
    ...
    g_native_conf['anieditor_recent_open_files'] = ret
...
function open_file_by_template_name(templateName)
    if not g_uisystem.is_ani_template_valid(templateName) then
        printf('open_file_by_template_name [%s] not valid', str(templateName))
        return
    end

    g_editor_panel:ShowActionAniEditor()
    ...
    if p then
        ...
        local panel = g_editor_panel.anieditorPanel:NewAniEditorPanel(templateName) 
        table.insert(_openPanelList, panel)
        table.insert(_openPanelOrder, 1, panel)
        _addRecentFile(panel)
        _curPanel = panel

        g_logicEventHandler:Trigger('anieditor_multi_doc_status_changed')
        return panel
    end
end
...
function new_file()
    _curPanel = g_editor_panel.anieditorPanel:NewAniEditorPanel(nil)
...
function save_file(panel, templateName)
    ...
    if templateName == nil then
        templateName = win_save_file('保存配置', g_logic_editor.get_project_ani_template_path())
    end
    ...
    _addRecentFile(panel)
    g_uisystem.reload_ani_template(templateName)
    g_logicEventHandler:Trigger('anieditor_multi_doc_status_changed')
...
function save_template_conf(panel)
    local conf, saveFullPath = panel:GetAniConfig(), panel:GetSaveFilePath()
    assert(g_uisystem.is_ani_template_valid(conf))
    ...
    local function _wc(conf, nTab)
        _ws('{\n')
        local keys = table.keys(conf)
        table.arr_bubble_sort(keys, function(v1, v2)
            if v1 == 'type_name' then
                return true
            elseif v2 == 'type_name' then
                return false
            elseif v1 == 'child_list' then
                return false
            elseif v2 == 'child_list' then
                return true
            else
                return tostring(v1) < tostring(v2)
            end
        ...
        for _, k in ipairs(keys) do
            local attrV = conf[k]
            if k == 'child_list' and table.is_empty(attrV) then
                break
            end
            _wt(nTab)
            _w(tostring(k))
    ...
    g_uisystem.reload_ani_template(panel:GetTemplateName())
end
