...
function EditCtrl:on_init_ui()
    ...
    self.btn.OnClick = function()
        local templateName = self._editConf and self._editConf[self._validateParm['edit_template']] or self._validateParm['edit_template']
        local curPanelLayer = g_multi_doc_manager.get_cur_open_panel():get_layer()
        g_panel_mgr.show_multiple_with_parent('uieditor.dlg_edit_template_attr_panel', curPanelLayer, templateName, self._data, self.btn, function(data)
            -- print('~~~~ dlg_edit_template_attr_panel callback', data)
