...
function cc.Live2DSprite:Create(modelPath, isAsyn)
    ...
    if not self:_parseModelConfig(modelPath) then
        return nil
    end
    local ret = nil
    if isAsyn then
        ret = self:create(self.mocPath):CastTo(self):_init()
        ret:LoadTextureAsyn()
    else
        ret = self:create(self.mocPath, self.texturePaths):CastTo(self):_init()
    end
...
function cc.Live2DSprite:LoadTextureAsyn(callback)
    if not self.texturePaths or #self.texturePaths <= 0 then
        return
    end
    local loadedCount = 0
    local totalCount = #self.texturePaths
    for _, path in ipairs(self.texturePaths) do
        utils_add_image_async(path, function(tex)
            if self:IsValid()then
                ...
                if loadedCount == totalCount then
                    ...
                    self:SetTexturePaths(self.texturePaths)
                end
            end
        end)
    end
...
function cc.Live2DSprite:SetModelPath(newModelPath, isAsyn)
    if self.modelPath == newModelPath then
        return
    end
    ...
    if not self:_parseModelConfig(newModelPath) then
        return
    end

    local originWidth, originHeight = self:GetContentSize()
    self:SetMocPath(self.mocPath)
    if isAsyn then
...
function cc.Live2DSprite:_parseModelConfig(modelPath)
...
function cc.Live2DSprite:PlayMotion(motion, isLoop, isLoopFadeIn, offset, callback)
    local motionFiles = self.motions[motion]
