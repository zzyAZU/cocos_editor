...
local default_combobox_template = 'ctrls/combobox/ctrl_combobox'
local deault_combobox_popup_layer_template = 'ctrls/combobox/ctrl_combobox_popup_layer'

-- override
function CCCombobox:Create()
    return g_uisystem.load_template_create(default_combobox_template):CastTo(self):_init()
end
...
function CCCombobox:_registerInnerEvent()
    ...
    self:_regInnerEvent('OnMouseMoveInside')
...
function CCCombobox:_createMenuList(itemData, parentItem, relativeNode)
    ...
    local popupLayer = g_uisystem.load_template_create(deault_combobox_popup_layer_template, parent)
    ...
    for i, info in ipairs(itemData) do
        ...
        if _bgActivated_ then
            ...
            item.OnMouseMove = function(bMoveInside, pos, bFirst)
                ...
                if bMoveInside and is_table(info._itemData) then
                   self:_createMenuList(info._itemData, itemData, item)
                elseif bFirst then
                    self.eventHandler:Trigger('OnMouseMoveInside', text, itemName)
                end
            end
        end
