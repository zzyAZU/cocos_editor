...
local function _nodeForTouch(pt)
    for i, node in ipairs(_nodeZorderList) do
...
local function _removeMouseEvent(node)
...
local function _initMouseEvent()
    ...
    local winHeight = g_director:getWinSize().height
    ...
    mouseListener:registerScriptHandler(function(event)
        ...
        _timerCall(function()
            lastMovePos.y = winHeight + lastMovePos.y
